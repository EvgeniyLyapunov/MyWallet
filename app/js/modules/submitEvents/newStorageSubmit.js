...
function newStorageSubmit() {
  tippy('#tippy-base-storage', {
    content:
      'Не обязательная опция. Базовый кошелёк - кошелёк, на основе баланса которого вы создаёте новый кошелёк. При выборе этой опции, уменьшение суммы данного кошелька будет уменьшать сумму базового. Операции в базовом кошельке не будут влиять на сумму этого кошелька, пока сумма базового больше.',
  });
  ...
  validate
    ...
    .addRequiredGroup(
      '#new-storage-radio-group',
      'Нужно выбрать тип денег в кошельке'
    );
  ...
  async function newStorageFormSubmit() {
    ...
    newStorageData.lastModifiedDate = now
      .toLocaleString('ru', options)
      .toString();
    // записываем позицию кошелька в списке
    data.sort((a, b) => a.position - b.position);
    newStorageData.position = +data[data.length - 1].position + 1;
